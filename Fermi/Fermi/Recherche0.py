...
""" la question des bornes se pose, il faut trouver le 0 de la fonction
    ...
    prendre comme borne [tmin,pi/w*n] et si il n'existe pas de import numpy as np
import matplotlib.pyplot as plt
import matplotlib as mplsolution 
...
#Pour le mur
x_equ_mur = 2 #position moyenne du mur
A = 1          #Amplitude des oscillation
w = 4          #Fréquence

#Pour la balle
x_balle = 0  #Pos initiale balle Tout va bien
v = 5         #vitesse init balle Tout va bien
...
while(t<Tfin):       
    ...
    t = recherche( Contact_raquette , ContactPrime_raquette,tmin,borneinf \
                  ,bornesup,eps ,  t ,  v = v ,  w = w ,   A = A \
                  ,  x0_mur =x_equ_mur ,x0_balle = x_balle,t0=tavant )
...
scat = ax.scatter(x_evolution[0], 0)
line = ax.axvline(x=XMur(t_evolution[0],w,A,x_equ_mur), ymin=-0.15, ymax=15,color='black') 
